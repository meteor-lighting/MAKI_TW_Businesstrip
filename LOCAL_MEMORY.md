...
- **2026-04-02 ~ 2026-04-14 近期異動彙整**
  - **核心應用與元件：**
    - 新增 `Copy Report` (複製報告) 功能，支援從 Dashboard 完整拷貝包含所有 14 張費用明細子表單的整份報告。
    - 開發 `CountryMultiSelect` (國家多選下拉選單) 元件，串接 REST Countries API 取代手動輸入，並具備搜尋、常用國家置頂與多標籤樣式。
  - **細節調整與除錯：**
    - 將各個頁面的日期強制統一為 `YYYY/MM/DD` 以修復部份瀏覽器顯示 `NaN/NaN/NaN` 的問題。
    - 全面統一語言包及 UI 名稱，將舊稱「午餐與學費」合併更名為「Lunch & Learn」。
    - 在報告總結 (Report Summary) 介面中，將「交通運輸明細」順序移動至「停車費明細」之下展示。
  - **Dashboard (我的報告) 強化：**
    - 新增**動態搜尋列功能**，可在客戶端即時對「報告編號、報告名稱、用戶名稱、商旅日期、商旅天數、狀態」進行文字與日期檢索關聯過濾。
  - **History (歷史資料) 系統強化：**
    - 擴展「操作」欄位，當為「全部 (All Reports)」分類時新增**預覽 PDF**按鈕 (直接跳轉至報告總結明細)。
    - 全分類模式下，使用新的 `CountryMultiSelect` 以取代傳統地區輸入框。
    - 當選擇「機票 (Flight)」分類時，動態新增「出發地、抵達地、幣別」三個搜尋條件。
    - 當選擇「住宿 (Accommodation)」分類時，動態新增「幣別」搜尋條件。
    - **[Bug Fix]** 修正 `queryHistoryData` 後端資料對應延遲之錯誤，把「映射真實姓名 (`userMap`)」邏輯移到查詢過濾之前，解決依「員工編號 / 用戶名稱」搜尋無法比對出報告的問題。
    - **[Bug Fix]** 取消「所有子分類搜尋必須預先嚴格受限於母報告出差國家」之機制，使得在住宿、機票下可以直接透過「細項地區」（如：Bangkok）準確對此筆花費進行過濾而不會被父節點隱蔽。
