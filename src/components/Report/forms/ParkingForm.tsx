...
export default function ParkingForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: ParkingFormProps) {
    const { t } = useTranslation();
    const { register, handleSubmit, watch, setValue, control, formState: { errors } } = useForm<ParkingFormData>({
        defaultValues: {
            currency: 'TWD',
            amount: '',
            rate: 1,
    ...
    const onSubmit = async (data: ParkingFormData) => {
        ...
        try {
            ...
            alert('Error saving parking expense: ' + e);
        } finally {
            setLoading(false);
