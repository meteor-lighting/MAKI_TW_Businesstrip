...
export default function RentalCarForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: RentalCarFormProps) {
    ...
    useEffect(() => {
        ...
        const fetchRate = async () => {
            ...
            if (rentalStartDate && rentalEndDate) {
                const inDate = new Date(rentalStartDate);
                const outDate = new Date(rentalEndDate);
            ...
            let targetDate = rentalStartDate;
            if (tripStartDate && tripStartDate !== '-' && tripStartDate !== '') {
                const itemD = new Date(rentalStartDate);
    ...
    }, [currency, personalAmount, advanceAmount, peopleCount, rentalStartDate, rentalEndDate, tripStartDate, setValue, headerRate]);
    ...
    const onSubmit = async (data: RentalCarFormData) => {
        ...
        try {
            ...
            if (data.rentalStartDate && data.rentalEndDate) {
                const inDate = new Date(data.rentalStartDate);
                const outDate = new Date(data.rentalEndDate);
                if (!isNaN(inDate.getTime()) && !isNaN(outDate.getTime())) {
                    const diffTime = outDate.getTime() - inDate.getTime();
                    n = Math.max(1, Math.round(diffTime / (1000 * 60 * 60 * 24)));
                }
            ...
            alert('Error saving rental car: ' + e);
        } finally {
            setLoading(false);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="relative space-y-4 p-4 border rounded bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700">{t('rental_start_date')} (YYYY/MM/DD)</label>
                    <input
                        type="date"
                        ...
                        className={`mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100 [&:-webkit-autofill]:shadow-[0_0_0_1000px_white_inset] [&:-webkit-autofill]:!bg-white ${errors.rentalStartDate ? 'border-red-500' : ''}`}
                    />
                    {errors.rentalStartDate && <span className="text-red-500 text-sm">{errors.rentalStartDate.message}</span>}
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700">{t('rental_end_date')} (YYYY/MM/DD)</label>
                    <input
                        type="date"
                        ...
                        disabled={loading || disabled}
                        className={`mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100 [&:-webkit-autofill]:shadow-[0_0_0_1000px_white_inset] [&:-webkit-autofill]:!bg-white ${errors.rentalEndDate ? 'border-red-500' : ''}`}
                    />
                    {errors.rentalEndDate && <span className="text-red-500 text-sm">{errors.rentalEndDate.message}</span>}
                </div>
                <div>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700">{t('rental_company')}</label>
                    <input type="text" {...register('rentalCompany')} disabled={loading || disabled} className="mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100" />
                </div>
                <div>
