...
export default function LuggageFeeForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: LuggageFeeFormProps) {
    const { t } = useTranslation();
    const { register, handleSubmit, watch, setValue, control, formState: { errors } } = useForm<LuggageFeeFormData>({
        defaultValues: {
            currency: 'TWD',
            amount: '',
            rate: 1,
    ...
    const onSubmit = async (data: LuggageFeeFormData) => {
        setLoading(true);
        ...
        try {
            ...
            alert('Error saving luggage_fee expense: ' + e);
        } finally {
            setLoading(false);
