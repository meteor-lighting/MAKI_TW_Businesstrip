...
export default function HandingFeeForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: HandingFeeFormProps) {
    const { t } = useTranslation();
    const { register, handleSubmit, watch, setValue, control, formState: { errors } } = useForm<HandingFeeFormData>({
        defaultValues: {
            currency: 'TWD',
            amount: '',
            rate: 1,
    ...
    const onSubmit = async (data: HandingFeeFormData) => {
        setLoading(true);
        ...
        try {
            ...
            alert('Error saving handing fee expense: ' + e);
        } finally {
            setLoading(false);
