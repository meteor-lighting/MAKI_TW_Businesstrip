...
interface OthersFormData {
    date: string;
    subCategory: string; // Corresponds to '分類'
    region: string;
    currency: string;
...
export default function OthersForm({ reportId, headerRate, onSubmitSuccess, onLoadingChange, disabled = false }: OthersFormProps) {
    ...
    const onSubmit = async (data: OthersFormData) => {
        ...
        try {
            ...
            alert('Error saving others expense: ' + e);
        } finally {
            setLoading(false);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="relative space-y-4 p-4 border rounded bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input
                        type="text"
                        {...register('subCategory', { required: t('please_enter_category') })}
                        disabled={loading || disabled}
                        className={`mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100 [&:-webkit-autofill]:shadow-[0_0_0_1000px_white_inset] [&:-webkit-autofill]:!bg-white ${errors.subCategory ? 'border-red-500' : ''}`}
                    />
