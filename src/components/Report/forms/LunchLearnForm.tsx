...
export default function LunchLearnForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: LunchLearnFormProps) {
    ...
    const onSubmit = async (data: LunchLearnFormData) => {
        setLoading(true);
        ...
        try {
            ...
            alert('Error saving Lunch & Learn expense: ' + e);
        } finally {
            setLoading(false);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="relative space-y-4 p-4 border rounded bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700">{t('headcount')}</label>
                    <input type="number" {...register('headcount', { required: true, min: 1 })} disabled={loading || disabled} className={`mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100 ${errors.headcount ? 'border-red-500' : ''}`} />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700">{t('dealer')}</label>
                    <input type="text" {...register('dealer', { required: true })} disabled={loading || disabled} className={`mt-1 block w-full rounded border-gray-300 shadow-sm p-2 disabled:bg-gray-100 ${errors.dealer ? 'border-red-500' : ''}`} />
                </div>
            </div>
