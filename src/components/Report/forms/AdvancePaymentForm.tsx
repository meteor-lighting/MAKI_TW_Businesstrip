...
interface AdvancePaymentFormData {
    date: string;
    region: string;
    currency: string;
...
export default function AdvancePaymentForm({ reportId, headerRate, tripStartDate, onSubmitSuccess, onLoadingChange, disabled = false, editingItem, onCancelEdit }: AdvancePaymentFormProps) {
    ...
    const onSubmit = async (data: AdvancePaymentFormData) => {
        ...
        try {
            ...
            alert('Error saving advance payment: ' + e);
        } finally {
            setLoading(false);
